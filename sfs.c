...
void display(char *fname)
{
    ...
    i_type = _inode_table[CD_INODE_ENTRY].TT[0];
    if (i_type == 'F')
    {
        ...
        for (int j = 0; j < 4; j++)
        {
            if (directories[j].F == '1')
            {
                if (strcmp(directories[j].fname, fname) == 0)
                {
                    ...
                    i_type = _inode_table[tempiType].TT[0];
                    if (i_type == 'D')
                    {
                        isDir = 1;
                        continue;
    ...
    if (isDir == 1 && target_inode == -1)
    {
        printf("Given File name is a directory. Cannot be open.\n");
        return;
    }
    else if (target_inode == -1)
    {
        printf("File not found in the current directory\n");
        return;
    }
    else
    {
...
void create(char *fname)
{
    char itype;
    int blocks[3];
    int emptyBlockIndex = -1, dirEntryIndex = -1;
    int createNewBlock = 0;
    int newInode;
    ...
    _directory_entry new_directory[4];
    readSFS(blocks[emptyBlockIndex], (char *)new_directory);

    new_directory[dirEntryIndex].F = '1';
    strncpy(new_directory[dirEntryIndex].fname, fname, 252);
    itos(new_directory[dirEntryIndex].MMM, newInode, 3);

    writeSFS(blocks[emptyBlockIndex], (char *)new_directory);
